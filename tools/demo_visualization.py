...
# ---------------------------------------------
# Modified from https://github.com/zhiqi-li/BEVFormer
# ---------------------------------------------
...
def visualize_tracking_sample(nusc: NuScenes,
                               sample_token: str,
                               gt_boxes: EvalBoxes,
    ...
    # Show GT boxes.
    for box in boxes_gt:
        box.render(ax, view=np.eye(4), colors=COLOR_MAP['green'], linewidth=2)
    ...
    for box in boxes_est:
        ...
        if box.score >= conf_th:
            box.render(ax, view=np.eye(4), colors=COLOR_MAP['light_blue'], linewidth=1)
...
def lidiar_render(sample_token, data, out_path=None):
    ...
    for content in bbox_anns:
        # if content['detection_name'] not in ['car', 'truck', 'bus', 'trailer', 
    ...
    visualize_tracking_sample(nusc, sample_token, gt_annotations, pred_annotations, savepath=out_path+'_bev')
            ...
            if with_anns:
                for box in boxes_pred:
                    ...
                    box.render(ax[j, ind], view=camera_intrinsic, normalize=True, \
                        colors=color, linestyle='dashed', linewidth=1.2)
                for box in boxes_gt:
                    c = np.array(get_color(box.name)) / 255.0
...
def parse_args():
    ...
    parser.add_argument('--data_infos_path', type=str, default='./data/nuscenes/tracking-mini_infos_val.pkl')
...
if __name__ == '__main__':
    ...
    #     indexes = sorted([fname.split('_')[0] for fname in fig_names if fname.endswith('png')])
