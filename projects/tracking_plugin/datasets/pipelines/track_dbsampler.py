...
class TrackDBSampler(DataBaseSampler):
    ...
    def prune_collisions(
        self, gt_bboxes_3d: np.ndarray, instance_inds: np.ndarray, sampled: List[Dict]
    ) -> List[Dict]:
        ...
        for i, s in enumerate(sampled):
            if s["instance_ind"] in instance_inds:
                breakpoint()  # check behaviour
                coll_mat[i, instance_inds == s["instance_ind"]] = True

        # select valid samples based on collisions
        valid_samples = []
        for i, s in enumerate(sampled):
            if not coll_mat[i].any():  # if no collisions
